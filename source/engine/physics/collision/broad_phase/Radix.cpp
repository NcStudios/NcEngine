...
#define CREATE_HISTOGRAMS(type, buffer)														\
	...
	{																						\
		...
		{																					\
			mNbHits++;																		\
			for(unsigned i=0;i<nb;i++)	mRanks[i] = i;										\
			return *this;																	\
		}																					\
	}																						\
	else																					\
...
bool RadixSort::Resize(unsigned nb)
{
	...
	// Get some fresh one
	mRanks	= new unsigned[nb];
	mRanks2	= new unsigned[nb];

	return true;
}

inline void RadixSort::CheckResize(unsigned nb)
{
 ...
 */
///////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
RadixSort& RadixSort::Sort(const unsigned* input, unsigned nb, bool signedvalues)
{
	// Checkings
	...
	if(!signedvalues)	{ CREATE_HISTOGRAMS(unsigned, input);	}
	...
	if(signedvalues)
	{
		...
		for(unsigned i=128;i<256;i++)	NbNegativeValues += h3[i];	// 768 for last histogram, 128 for negative part
	}
	...
	for(unsigned j=0;j<4;j++)
	{
		...
		if(PerformPass)
		{
			...
			if(j!=3 || !signedvalues)
			{
				...
				// Create offsets
				mOffset[0] = 0;
				for(unsigned i=1;i<256;i++)		mOffset[i] = mOffset[i-1] + CurCount[i-1];
			}
			else
			{
				...
				for(unsigned i=1;i<128;i++)		mOffset[i] = mOffset[i-1] + CurCount[i-1];	// 1 to 128 for positive numbers

				// Fixing the wrong place for negative values
				mOffset[128] = 0;
				for(auto i=129;i<256;i++)			mOffset[i] = mOffset[i-1] + CurCount[i-1];
			}
			...
			if(INVALID_RANKS)
			{
				for(unsigned i=0;i<nb;i++)	mRanks2[mOffset[InputBytes[i<<2]]++] = i;
				VALIDATE_RANKS;
			}
			else
			{
				unsigned* Indices		= mRanks;
				unsigned* IndicesEnd	= &mRanks[nb];
			...
			unsigned* Tmp	= mRanks;	mRanks = mRanks2; mRanks2 = Tmp;
		}
	}
	return *this;
}
 ...
 */
///////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
RadixSort& RadixSort::Sort(const float* input2, unsigned nb)
{
	// Checkings
	...
	for(unsigned j=0;j<4;j++)
	{
		...
		else
		{
			...
			if(PerformPass)
			{
				...
				mOffset[255] = 0;
				for(unsigned i=0;i<127;i++)		mOffset[254-i] = mOffset[255-i] + CurCount[255-i];	// Fixing the wrong order for negative values
				for(unsigned i=128;i<256;i++)	mOffset[i] += CurCount[i];							// Fixing the wrong place for negative values
				...
				if(INVALID_RANKS)
				{
					...
					{
						unsigned Radix = input[i]>>24;							// Radix byte, same as above. AND is useless here (unsigned).
				...
				else
				{
					for(unsigned i=0;i<nb;i++)
					{
						unsigned Radix = input[mRanks[i]]>>24;							// Radix byte, same as above. AND is useless here (unsigned).
						// ### cmp to be killed. Not good. Later.
			...
			else
			{
				...
				if(UniqueVal>=128)
				{
					...
					{
						// ###Possible?
						for(unsigned i=0;i<nb;i++)	mRanks2[i] = nb-i-1;
						VALIDATE_RANKS;
					}
					else
					{
						for(unsigned i=0;i<nb;i++)	mRanks2[i] = mRanks[nb-i-1];
					}
 ...
 */
///////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
unsigned RadixSort::GetUsedRam() const
{
	unsigned UsedRam = sizeof(RadixSort);
#ifndef RADIX_LOCAL_RAM
