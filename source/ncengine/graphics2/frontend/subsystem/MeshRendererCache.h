...
class InstanceCache
{
    public:
        ...
        {
            const auto index = m_buffer.emplace(DirectX::XMMATRIX{}, materialId);
        ...
        {
            return !m_buffer.has_dirty_state();
        }
        ...
        void UpdateMatrices(ecs::ExplicitEcs<Transform> ecs)
        {
            ...
            for (const auto index : m_buffer.dirty_indices())
            {
                const auto entity = m_entities[index];
                if (entity.IsStatic())
                {
                    const auto& transform = ecs.Get<Transform>(entity);
                    m_buffer[index].modelMatrix = transform.TransformationMatrix();
                }
            }

            // For each dynamic instance, update matrix and mark dirty
            for (auto [i, data] : std::views::enumerate(m_buffer))
            {
                ...
                const auto& transform = ecs.Get<Transform>(entity);
                data.modelMatrix = transform.TransformationMatrix();
                m_buffer.mark_dirty(static_cast<uint32_t>(i));
            }
        }
    ...
    private:
        HostBufferCache<MeshRendererData> m_buffer;
