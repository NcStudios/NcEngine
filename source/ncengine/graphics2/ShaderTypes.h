...
struct LightData
{
    ...
    LightData(const Vector3& diffuseCol,
        ...
        : diffuseColor{diffuseCol},
          ...
          cascadeStartIndex{cascadeStartIndex_},
    ...
    uint32_t cascadeStartIndex = 0u;   // Index into CascadeData buffer
    uint32_t cascadeCount = 0u;        // 0 = legacy single shadow map, >0 = CSM
    uint32_t padding[3] = {1u, 1u, 1u};
};

constexpr uint32_t MaxCascadeCount = 4u;

struct CascadeData
{
    DirectX::XMMATRIX viewProjection = DirectX::XMMATRIX{};
    float splitDepth = 0.0f;           // View-space far depth for this cascade
    float texelSize = 0.0f;            // World-space texel size for bias
