...
namespace nc
{
    ...
    struct Key 
    {
        ...
        constexpr Key(const char* input) noexcept;
        Key(const std::string input)     noexcept;
        Key(const Key& input)            noexcept;

        operator size_t()               const noexcept { return value; }
        explicit operator const char*() const noexcept { return text; }
        explicit operator std::string() const noexcept { return std::string{text}; }
        explicit operator bool()        const noexcept { return text != nullptr; }
    ...
    {
        KeyValue value;
        Key keys[KeyListMazSize]; // Store at most 4 keys, that should be enough for debug validation
