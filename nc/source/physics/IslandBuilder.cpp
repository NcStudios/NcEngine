...
{
    auto IslandBuilder::FindIslands(uint32_t totalColliderCount, std::span<const BroadEvent> broadPhysicsEvents) -> std::vector<std::span<IslandElement>>
    {
    ...
    void IslandBuilder::Initialize(uint32_t colliderCount)
    {
    ...
    void IslandBuilder::AddPair(uint32_t indexA, uint32_t indexB)
    {
    ...
    auto IslandBuilder::FindRoot(uint32_t index) -> uint32_t
    {
    ...
    auto IslandBuilder::BuildIslands() -> std::vector<std::span<IslandElement>>
    {
         ...
         *  processed as distinct tasks. Maybe we should combine those here? */
