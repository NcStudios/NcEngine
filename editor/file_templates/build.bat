...
:CheckConfiguration
    if %CONFIGURATION%=="Debug" (
        set DEFINITIONS=-DCMAKE_BUILD_TYPE=Debug  -DNC_EDITOR_ENABLED=ON -DNC_DEBUG_RENDERING=ON -DNC_TESTS_ENABLED=ON -DVERBOSE_LOGGING_ENABLED=ON
        goto Run
    )
    if %CONFIGURATION%=="Debug-WithValidation" (
        set DEFINITIONS=-DCMAKE_BUILD_TYPE=Debug  -DNC_EDITOR_ENABLED=ON -DNC_DEBUG_RENDERING=ON -DNC_TESTS_ENABLED=ON -DVERBOSE_LOGGING_ENABLED=ON -DNC_USE_VALIDATION=ON
        goto Run
    )
    ...
    if %CONFIGURATION%=="Release-WithValidation" (
        set DEFINITIONS=-DCMAKE_BUILD_TYPE=Release -DNC_EDITOR_ENABLED=ON -DNC_DEBUG_RENDERING=ON -DNC_USE_VALIDATION=ON
        goto Run
    )
