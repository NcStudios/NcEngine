...
:CheckConfiguration
    if %CONFIGURATION%=="Debug-Vulkan" (
        set DEFINITIONS=-DCMAKE_BUILD_TYPE=Debug -DNC_EDITOR_ENABLED=ON -DNC_TESTS_ENABLED=ON -DVERBOSE_LOGGING_ENABLED=ON -DUSE_VULKAN=ON -DNC_USE_VALIDATION=ON
        goto Run
    )
    if %CONFIGURATION%=="Debug-NoValidation-Vulkan" (
