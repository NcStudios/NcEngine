...
#include "DDS.h"
#include "external/directx/tk/DDSTextureLoader.h"
#include "PlatformHelpers.h"
#include "external/mingw-std-threads/Winbase.h"
...
namespace DirectX
{
    namespace LoaderHelpers
    {
            ...
            size_t* bitSize) noexcept
        {
            ...
            // Get the file size
            mingw_stdthread::FILE_STANDARD_INFO_EX fileInfo;
            if (!mingw_stdthread::GetFileInformationByHandleEx(hFile.get(), mingw_stdthread::FileStandardInfoEx, &fileInfo, sizeof(fileInfo)))
            {
                return HRESULT_FROM_WIN32(GetLastError());
            }
    ...
    #undef ISBITMASK
        ...
        public:
            ...
            ~auto_delete_file()
            {
                if (m_handle)
                {
                    mingw_stdthread::FILE_DISPOSITION_INFO info = {};
                    info.DeleteFile = TRUE;
                    (void)mingw_stdthread::SetFileInformationByHandle(m_handle, mingw_stdthread::FileDispositionInfo, &info, sizeof(info));
                }
            }
