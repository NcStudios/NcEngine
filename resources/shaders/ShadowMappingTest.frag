...
float adjustContrast(float value, float contrastFactor) {
...
void main() 
{
	...
	if ((samplePos.x != 0.0f) && (samplePos.y != 0.0f)) {
		float dist = adjustContrast(length(texture(omniDirShadowMaps[0], samplePos).xyz) * 0.005, 1.0);
