...
Generally, don't store values from the registry across frames, except for Entities. Calls to Add, Remove, Sort and ReserveHeadroom for a type T may invalidate any existing span\<T\> or pointers/references to other Ts except:
...
* ToonRenderer: expects a path to a mesh and a material which requires three paths to textures and a tiling integer.
